...
def recent_file(list):
    list.sort(key=lambda x: getmtime(join(BLOCK_DIR, x)), reverse=True)
    most_recent_data = join(BLOCK_DIR, list[0])
    with open(most_recent_data, "r") as f:
        data = load(f)
    return data
